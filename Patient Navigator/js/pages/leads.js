...
export async function renderLeads(container) {
  container.innerHTML = `
    <div class="page-header">
      <div>
        ...
        <p class="header-subtitle" style="margin:4px 0 0">Finished the HopeBot profile on WhatsApp. Call them before they go cold.</p>
      </div>
  ...
  container.querySelector('#wl-refresh')?.addEventListener('click', load);
...
function visible() {
  return rows
    ...
    .filter(r => !searchQ || [r.name, r.phone, r.city, r.cancer_type, r.stage]
    ...
    ${list.map(r => `
      <div class="card" style="padding:12px 14px;${r.contacted ? 'opacity:.65' : ''}">
        <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
          ...
          <span class="due-meta">${formatRelativeTime(r.created_at)}</span>
